...
public class MathScreen extends ScreenAdapter {
    ...
    public MathScreen(SpeedDemo main){
        mainGame = main;
    }
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < SpeedDemo.STEPS; i++) {
            int m = (int) (Math.sin(random.nextFloat() * 512) * 255f + 256f);
            if(m >= 0 && m < 512)
                amounts[m]++;
        }
        ...
        batch.begin();
        font.draw(batch, Stringf.format("Math.sin() at %d FPS", Gdx.graphics.getFramesPerSecond()),
