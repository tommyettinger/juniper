...
public class DigitalSinSmoothScreen extends ScreenAdapter {
    ...
    public DigitalSinSmoothScreen(DistributorDemo main){
        mainGame = main;
    }
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < 0x40000; i++) {
            int m = (int) (TrigTools.sinSmooth(random.nextExclusiveFloat()) * 128 + 128);
            if(m >= 0 && m < 512)
                amounts[m]++;
        }
        ...
        batch.begin();
        font.draw(batch, Stringf.format("TrigTools.sinSmooth() at %d FPS", Gdx.graphics.getFramesPerSecond()),
