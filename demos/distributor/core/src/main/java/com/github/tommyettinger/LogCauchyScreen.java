...
public class LogCauchyScreen extends ScreenAdapter {
    ...
    public void show() {
        font = mainGame.font;
        try  {
            dist = new LogCauchyDistribution(mainGame.random, a, b);
        } catch (IllegalArgumentException ignored) {
            dist = new LogCauchyDistribution(mainGame.random, 0.0, 1.0);
        }
    ...
    public void render(float delta) {
        ...
        batch.begin();
        font.draw(batch, Stringf.format("LogCauchyDistribution with A=%.3f, B=%.3f; median=%.3f at %d FPS",
