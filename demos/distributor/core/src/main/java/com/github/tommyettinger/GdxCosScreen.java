...
public class GdxCosScreen extends ScreenAdapter {
    ...
    public GdxCosScreen(DistributorDemo main){
        mainGame = main;
    }
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < 0x40000; i++) {
            int m = (int) (MathUtils.cos(random.nextExclusiveFloat()) * 256 + 128);
            if(m >= 0 && m < 512)
                amounts[m]++;
        }
        ...
        batch.begin();
        font.draw(batch, Stringf.format("MathUtils.cos() at %d FPS", Gdx.graphics.getFramesPerSecond()),
