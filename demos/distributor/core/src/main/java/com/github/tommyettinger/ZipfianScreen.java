...
public class ZipfianScreen extends ScreenAdapter {
    ...
    public void show() {
        font = mainGame.font;
        try {
            dist = new ZipfianDistribution(mainGame.random, (long) a, b);
        }catch (IllegalArgumentException ignored) {
            dist = new ZipfianDistribution(mainGame.random, 16L, 0.5);
        }
    ...
    public void render(float delta) {
        ...
        for (int x = 0; x < 512; x++) {
            ...
            renderer.color(color);
            renderer.vertex(x, (amounts[x >> 4] / iterations), 0);
        }
        ...
        batch.begin();
        font.draw(batch, Stringf.format("ZipfianDistribution with A=%.3f, B=%.3f, zeta=%.6f; at %d FPS",
