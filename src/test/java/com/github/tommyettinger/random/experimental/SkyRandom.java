...
/**
 */
public class SkyRandom extends EnhancedRandom {
	@Override
	...
	/**
	 * Creates a new SkyRandom with a random state.
	 */
	public SkyRandom() {
	...
	/**
	 * Creates a new SkyRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public SkyRandom(long seed) {
		setSeed(seed);
	}

	/**
	 * Creates a new SkyRandom with the given two states; all {@code long} values are permitted.
	 * These states will be used verbatim for stateA and stateB. Both stateC and stateD will be assigned 1.
	 *
	 * @param stateA any {@code long} value
	 * @param stateB any {@code long} value
	 */
	public SkyRandom(long stateA, long stateB) {
		this.stateA = stateA;
		this.stateB = stateB;
		this.stateC = 1L;
		this.stateD = 1L;
	...
	/**
	 * Creates a new SkyRandom with the given four states; all {@code long} values are permitted.
	 ...
	 */
	public SkyRandom(long stateA, long stateB, long stateC, long stateD) {
		this.stateA = stateA;
		this.stateB = stateB;
		this.stateC = stateC;
		this.stateD = stateD;
	...
	public long nextLong () {
		...
		long x = a + (c << 31 | c >>> 33);
		x ^= x >>> 27;
		x = x * 0x3C79AC492BA7B653L + (b << 17 | b >>> 47);
		x ^= x >>> 33;
		x = x * 0x1C69B3F74AC4AE35L + (d << 49 | d >>> 15);
		return x ^ x >>> 27;
	}
	...
	@Override
	public SkyRandom copy () {
		return new SkyRandom(stateA, stateB, stateC, stateD);
	}
	...
	public boolean equals (Object o) {
		...
		SkyRandom that = (SkyRandom)o;

		return stateA == that.stateA && stateB == that.stateB && stateC == that.stateC && stateD == that.stateD;
	}

	public String toString () {
		return "SkyRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L, stateC=" + (stateC) + "L, stateD=" + (stateD) + "L}";
	}

//	public static void main(String[] args) {
//		SkyRandom random = new SkyRandom(1L);
