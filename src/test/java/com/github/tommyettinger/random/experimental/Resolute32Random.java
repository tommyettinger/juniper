...
/**
 * An experiment to try to improve {@link com.github.tommyettinger.random.Respite32Random}.
 * This should be super-sourced on GWT, using {@code Math.clz32()} and {@code Math.imul()}.
 *
 */
public class Resolute32Random extends EnhancedRandom {
	...
	/**
	 * Creates a new Resolute32Random with a random state.
	 */
	public Resolute32Random() {
		this(EnhancedRandom.seedFromMath());
	}

	/**
	 * Creates a new Resolute32Random with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public Resolute32Random(long seed) {
		super(seed);
		setSeed(seed);
	}

	/**
	 * Creates a new Resolute32Random with the given three states. All {@code int} values are permitted.
	 *
	 * @param stateA any {@code int} value
	 * @param stateB any {@code int} value
	 * @param stateC any {@code int} value
	 */
	public Resolute32Random(int stateA, int stateB, int stateC) {
		super(stateA);
		this.stateA = stateA;
		this.stateB = stateB;
		this.stateC = stateC;
...
//		Resolute32Random random = new Resolute32Random(1L);
