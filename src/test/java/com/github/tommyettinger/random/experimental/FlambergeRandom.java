...
/**
 * A simple generator that combines an additive counter (Weyl sequence) with a 64-bit LFSR for its two
 * state transitions, and outputs a value after using an ARX-only mixer.
 * <br>
 * This is the same as FlamencoRandom, but uses a Galois LFSR instead of a Xorshift generator with shifts 9 and 7.
 * The Galois LFSR uses the constant {@code 0xfeedbabedeadbeefL}, which is a valid full-period LFSR constant while still
 * only using English words in a hex constant.
 */
public class FlambergeRandom extends EnhancedRandom {
	...
	/**
	 * Creates a new FlambergeRandom with a random state.
	 */
	public FlambergeRandom() {
		super();
	...
	/**
	 * Creates a new FlambergeRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public FlambergeRandom(long seed) {
		super(seed);
		setSeed(seed);
	}

	/**
	 * Creates a new FlambergeRandom with the given two states; all {@code long} values are permitted except 0 for
	 ...
	 */
	public FlambergeRandom(long stateA, long stateB) {
		super(stateA);
		this.stateA = stateA;
		this.stateB = stateB;
		if (stateA == 0L)
	...
	@Override
	public FlambergeRandom copy() {
		return new FlambergeRandom(stateA, stateB);
	}
	...
	public boolean equals(Object o) {
		...
		FlambergeRandom that = (FlambergeRandom) o;
	...
	public String toString() {
		return "FlambergeRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L}";
	}

	public static void main(String[] args) {
		FlambergeRandom random = new FlambergeRandom(1L);
		{
