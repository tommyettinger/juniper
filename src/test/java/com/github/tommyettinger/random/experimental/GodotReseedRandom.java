 ...
 * <br>
 * This generator passes ICE and IICE tests for correlation between similar initial seed and increments by
 * pre-processing the inputs to {@link #pcg32_srandom_r(long, long)}. Note that this is not the same treatment that
 * other generators get; most don't have a specialized state-changing operation that applies to the initial states that
 * the ICE and IICE tests are given.
 */
public class GodotReseedRandom extends EnhancedRandom {
	...
	public void seedWrapper(long initialState, long initialSeq) {
		// This works! No correlations I can find with an FFT.
		// 0x9E3779B97F4A7C15L is 2 to the 64 divided by the golden ratio, rounded down to an odd number.
		// 0xBEA225F9EB34556DL is the multiplier "C" from the MX3 unary hash.
		// Incorporating (the low 63 bits of) initialSeq into the state is vital for breaking up patterns.
		pcg32_srandom_r(initialState * (initialSeq * 2L + 0x9E3779B97F4A7C15L), initialSeq * 0xBEA225F9EB34556DL);
		//  This also works, but needs more work to get decorrelated.
...
//			initialSeq);
		// Correlations between similar seeds; a grid of generators detectably does not produce white noise.
//		pcg32_srandom_r(initialState, initialSeq);
	}
	...
	/**
	 * Creates a new GodotReseedRandom with a fixed state.
	 */
	public GodotReseedRandom() {
		this(DEFAULT_SEED, DEFAULT_INC);
	}

	/**
	 * Creates a new GodotReseedRandom with the given seed; all {@code long} values are permitted. The increment will be
	 * assigned a fixed value. The seed will not be used verbatim internally, but will be stored verbatim.
	 *
	 * @param seed any {@code long} value
	 */
	public GodotReseedRandom(long seed) {
		this(seed, DEFAULT_INC);
	}

	/**
	 * Creates a new GodotReseedRandom with the given two states; all {@code long} values are permitted for
	 ...
	 */
	public GodotReseedRandom(long p_seed, long p_inc) {
		super(p_seed);
		initialInc = p_inc;
		initialState = p_seed;
	...
	@Override
	public GodotReseedRandom copy() {
		GodotReseedRandom cpy = new GodotReseedRandom(initialState, initialInc);
	...
	public boolean equals(Object o) {
		...
		GodotReseedRandom that = (GodotReseedRandom) o;
	...
	public String toString() {
		return "GodotReseedRandom{" + "initialState=" + (initialState) + "L, initialInc=" + (initialInc) +
			"L, state=" + (state) + "L, inc=" + (inc) + "L}";
	}

//	public static void main(String[] args) {
//		GodotReseedRandom random = new GodotReseedRandom(1L);
//		{
...
//		}
//		random = new GodotReseedRandom(1L);
//		{
