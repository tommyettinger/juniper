...
/**
 */
public class TriQoaRandom extends EnhancedRandom {
	@Override
	...
	/**
	 * Creates a new TriQoaRandom with a random state.
	 */
	public TriQoaRandom() {
	...
	/**
	 * Creates a new TriQoaRandom with the given seed; all {@code long} values are permitted.
	 * The seed will be passed to {@link #setSeed(long)} to attempt to adequately distribute the seed randomly.
	 *
	 * @param seed any {@code long} value
	 */
	public TriQoaRandom(long seed) {
		setSeed(seed);
	}

	/**
	 * Creates a new TriQoaRandom with the given two states; all {@code long} values are permitted.
	 * These states will be used verbatim for stateA and stateB. stateC will be assigned 1.
	 *
	 * @param stateA any {@code long} value
	 * @param stateB any {@code long} value
	 */
	public TriQoaRandom(long stateA, long stateB) {
		this.stateA = stateA;
		this.stateB = stateB;
		this.stateC = 1L;
	}
	...
	/**
	 * Creates a new TriQoaRandom with the given three states; all {@code long} values are permitted.
	 ...
	 */
	public TriQoaRandom(long stateA, long stateB, long stateC) {
		this.stateA = stateA;
		this.stateB = stateB;
		this.stateC = stateC;
	}
	...
	public long nextLong() {
		long a = (stateA += stateA * stateA | 999999111L);
		long b = (stateB += stateB * stateB | 919919919L);
		long c = (stateC += stateC * stateC | 111999999L);
		return a ^ (b << 32 | b >>> 32) ^ (c << 16 | c >>> 48);
	}
	...
	public long previousLong() {
		...
		for (int bit = 0; bit < 64; bit++) {
			final long test = (((r + (r * r | 999999111L)) ^ a) & (-1L >>> ~bit));
			r ^= ((test | -test) >>> 63) << bit;
		}
		stateA = r;
		r = 0L;
		for (int bit = 0; bit < 64; bit++) {
			final long test = (((r + (r * r | 919919919L)) ^ b) & (-1L >>> ~bit));
			r ^= ((test | -test) >>> 63) << bit;
		}
		stateB = r;
		r = 0L;
		for (int bit = 0; bit < 64; bit++) {
			final long test = (((r + (r * r | 111999999L)) ^ c) & (-1L >>> ~bit));
			r ^= ((test | -test) >>> 63) << bit;
		}
		stateC = r;
	...
	public int next(int bits) {
		...
		long b = (stateB += stateB * stateB | 919919919L);
		long c = (stateC += stateC * stateC | 111999999L);
		return (int) (a ^ (b << 32 | b >>> 32) ^ (c << 16 | c >>> 48)) >>> (32 - bits);
	}

	@Override
	public TriQoaRandom copy() {
		return new TriQoaRandom(stateA, stateB, stateC);
	}
	...
	public boolean equals(Object o) {
		...
		TriQoaRandom that = (TriQoaRandom) o;

		return stateA == that.stateA && stateB == that.stateB && stateC == that.stateC;
	}

	public String toString() {
		return "TriQoaRandom{" + "stateA=" + (stateA) + "L, stateB=" + (stateB) + "L, stateC=" + (stateC) + "L}";
	}

	public static void main(String[] args) {
		EnhancedRandom random = new TriQoaRandom(1L);
		{
