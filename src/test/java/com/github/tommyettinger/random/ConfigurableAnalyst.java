...
import com.github.tommyettinger.random.experimental.Configurable4Random;
...
public class ConfigurableAnalyst extends ApplicationAdapter {
	...
	public static Configurable4Random[][] makeGrid(int width, int height) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				g[x][y] = new Configurable4Random(x, y, 1, 1);
			}
		}
		return g;
	}
	...
	static {
		randoms = new EnhancedRandom[1][][];
		randoms[0] = makeGrid(width, height);
	}
...
//        return BitConversion.intBitsToFloat(0xFE000000 | bt << 16);
		...
		InputAdapter input = new InputAdapter() {
			...
			public boolean keyDown(int keycode) {
				switch (keycode) {
					...
					case EQUALS:
					case ENTER:
						Configurable4Random.randomizeConfig();
						refreshGrid();
						title = Configurable4Random.class.getSimpleName()
							+ " selecting bit " + selectedBit;
						System.out.println(title);
						Configurable4Random.printConfig();
						if (!keepGoing) putMap();
						break;
					case RIGHT:
					case UP: // mode
	...
	public static void main(String[] arg) {
		...
		new Lwjgl3Application(new ConfigurableAnalyst(), config);
	}
}
